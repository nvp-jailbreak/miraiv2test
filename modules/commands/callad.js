...
module.exports.handleReply = async function({ api, args, event, handleReply, Users}) {
 ...
 switch(handleReply.type) {
   ...
   break;}
    case "calladmin": {
      api.sendMessage({ body: `🌃 Phản hồi từ admin ${name} đến bạn: \n ________________\n➢  ${event.body} \n________________\n⚔ Reply tin nhắn này để tiếp tục gửi tin nhắn về admin🐧`, mentions: [{tag: name, id : event.senderID}]}, handleReply.id, (e, data) => global.client.handleReply.push({
  name: this.config.name,
  author: event.senderID,
  messageID: data.messageID,
...
module.exports.run = async function({ api, event,Threads, args, Users }) {
 ...
 // const url = (api.getCurrentUserID(event.senderID));
  ...
  api.sendMessage(
    `📲 Đã chuyển lời nhắn của bạn đến admin.\n 💞 Vào lúc: ${gio}` ,
    event.threadID,
    () => {
    var idad = global.config.ADMINBOT;
    for(let ad of idad) {
        api.sendMessage(`🏙️Báo cáo từ: ${name}\n🏰 BoxName: ${namethread}\n🌃 ID box: ${idbox}\n________________\n🦟 Nội Dung: ${args.join(
            " "
          )}\n________________\n💐Time: ${gio}`,
          ad, (error, info) =>
